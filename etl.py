...
def process_song_file(cur, filepath):
    """To read json file into a Dataframe and to insert song record and artist record into the Table"""
    # open song file
...
def process_log_file(cur, filepath):
    """To read json file into a Dataframe and to load the log file into the Table"""
    # open log file
    df = pd.read_json(filepath, lines=True)
    ...
    # load user table
    user_df = df.sort_values(by='ts', ascending=True).loc[:,['userId','firstName', \
             'lastName', 'gender','level']].drop_duplicates('userId').dropna()
...
def main():
    """To connect to tje database and process the data files"""
